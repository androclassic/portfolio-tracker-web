...
export function computeDailyPositions(txs: TxLike[] | undefined): DailyPosition[] {
...
export function computeNotesByDayAsset(txs: TxLike[] | undefined): Map<string, string> {
...
export function useDailyPositions(txs: Tx[] | undefined) {
  const dailyPos = useMemo<DailyPosition[]>(() => computeDailyPositions(txs), [txs]);
  const notesByDayAsset = useMemo(() => computeNotesByDayAsset(txs), [txs]);
