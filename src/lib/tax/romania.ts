...
import { getHistoricalExchangeRateSync } from '@/lib/exchange-rates';
import {
  FIFOQueue,
  createFIFOQueue,
  addToFIFO,
    ...
    return {
      ...
      buyLots: s.buyLots.map((bl) => ({
        ...
        fundingSells: bl.fundingSells
          ? bl.fundingSells.map((fs) => ({ ...fs, amountUsd: fs.amountUsd * ratio }))
          : undefined,
      })),
    };
  };
...
function splitAssetMeta(meta: unknown, ratioUsed: number): { usedMeta: unknown; remainingMeta: unknown } {
  ...
  const used: AssetMeta = {
    buyLots: m.buyLots.map((bl) => ({
      ...
      fundingSells: bl.fundingSells
        ? bl.fundingSells.map((fs) => ({ ...fs, amountUsd: fs.amountUsd * ratioUsed }))
        : undefined,
    })),
  };
  const remaining: AssetMeta = {
    buyLots: m.buyLots.map((bl) => ({
      ...
      fundingSells: bl.fundingSells
        ? bl.fundingSells.map((fs) => ({ ...fs, amountUsd: fs.amountUsd * (1 - ratioUsed) }))
        : undefined,
    })),
  };
  return { usedMeta: used, remainingMeta: remaining };
...
function getFiatUsdAmount(tx: Transaction): { amountUsd: number; fxRateToUsd: number } {
  ...
  if (explicitUsd && explicitUsd > 0) {
    const fx = tx.quantity > 0 ? explicitUsd / tx.quantity : getHistoricalExchangeRateSync(asset, 'USD', txDateISO(tx.datetime));
    return { amountUsd: explicitUsd, fxRateToUsd: fx };
  }

  const fx = getHistoricalExchangeRateSync(asset, 'USD', txDateISO(tx.datetime));
  return { amountUsd: (tx.quantity || 0) * fx, fxRateToUsd: fx };
}
...
export function calculateRomaniaTax(
  ...
  for (const tx of sortedTxs) {
    ...
    if (tx.type === 'Buy') {
      ...
      const fundingSellsMap = new Map<number, { saleTransactionId: number; saleDatetime: string; asset: string; amountUsd: number }>();
      for (const e of removed) {
        ...
        if (!prev) {
          fundingSellsMap.set(s.saleTransactionId, {
            ...
            asset: s.asset,
            amountUsd: s.proceedsUsd,
          });
        } else {
          prev.amountUsd += s.proceedsUsd;
    ...
    if (tx.type === 'Withdrawal') {
      if (!isFiat) continue;
      const { amountUsd } = getFiatUsdAmount(tx);
        ...
        for (const bl of s.buyLots) {
          ...
          if (!existing) {
            ...
            if (bl.fundingSells && bl.fundingSells.length) {
              const m = new Map<number, { saleTransactionId: number; saleDatetime: string; asset: string; amountUsd: number }>();
              (existing.fundingSells || []).forEach((x: { saleTransactionId: number; saleDatetime: string; asset: string; amountUsd: number }) =>
                m.set(x.saleTransactionId, { ...x })
              );
              bl.fundingSells.forEach((x: { saleTransactionId: number; saleDatetime: string; asset: string; amountUsd: number }) => {
      ...
      if (txYear === targetYear) {
        taxableEvents.push({
          ...
          fiatAmountRon: withdrawActualUsd * usdToRonRate,
          costBasisUsd: totalCostBasis,
          costBasisRon: totalCostBasis * usdToRonRate,
          gainLossUsd: withdrawActualUsd - totalCostBasis,
          gainLossRon: (withdrawActualUsd - totalCostBasis) * usdToRonRate,
          sourceTrace,
  ...
  const totalWithdrawalsRon = totalWithdrawalsUsd * usdToRonRate;
  const totalCostBasisUsd = taxableEvents.reduce((sum, e) => sum + e.costBasisUsd, 0);
  const totalCostBasisRon = totalCostBasisUsd * usdToRonRate;
  const totalGainLossUsd = taxableEvents.reduce((sum, e) => sum + e.gainLossUsd, 0);
  const totalGainLossRon = totalGainLossUsd * usdToRonRate;
