...
After dependency installation (`npm install --legacy-peer-deps` + `npm run db:generate` + `npx prisma migrate deploy`), start the dev server with `npm run dev` on port 3000.

### Non-obvious caveats

- **`--legacy-peer-deps` required**: `npm install` fails without this flag due to a `nodemailer` peer dependency conflict between the project's pinned version and `next-auth`'s optional peer.
